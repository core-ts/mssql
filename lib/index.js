...
export function count(db, q, args) {
  return executeScalar(db, q, args);
}
...
export class SQLWriter {
  constructor(db, table, attributes, oneIfSuccess, toDB, buildParam, ver) {
    this.table = table;
  ...
  write(obj) {
    ...
    if (stmt) {
      if (this.exec) {
        ...
        if (this.oneIfSuccess) {
          return execute(this.db, stmt.query, stmt.params).then((ct) => (ct > 0 ? 1 : 0));
        }
        else {
          return execute(this.db, stmt.query, stmt.params);
        }
      }
    }
    else {
